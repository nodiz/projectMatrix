...
.set	clock = 4000000		; CURRENTLY 8 MHz

.def	char  = r0	; character (ASCII)
.def	_sreg = r1	; saves the status during interrupts
.def	_u 	= r2	; saves working reg u during interrupt				WE USE IT AS A WORKING REGISTER FOR RGB LED OUTPUT
...
.def	_w	= r17	; working register for interrupts	------------>	WE USE IT AS A SECOND WORKING REGISTER FOR SUBROUTINES TOO !
					; because our interrupts are only used to initialize a new mode => we don't care if we lose old datas stored in the registers
.equ	a	= 18
